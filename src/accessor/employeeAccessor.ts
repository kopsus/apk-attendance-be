...
const insertEmployee = async ({ companyId, email, hashedPassword }: { companyId: number, email: string, hashedPassword: string }): Promise<IStandardResponse> => {
    try {
        ...
        LoggerUtil.error(DOMAIN, `Got error when inserting employee: ${JSON.stringify(error)}`)
        throw error
    }
}
